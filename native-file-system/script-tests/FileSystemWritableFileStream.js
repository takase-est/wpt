...
directory_test(async (t, root) => {
  ...
  const writer2 = await handle.createWritable();
...
directory_test(async (t, root) => {
  ...
  assert_equals(await getFileSize(handle), 5);
}, 'createWritable({keepExistingData: true}): atomic writer initialized with source contents');
...
directory_test(async (t, root) => {
  ...
  assert_equals(await getFileContents(handle), 'bar');
  assert_equals(await getFileSize(handle), 3);
}, 'createWritable({keepExistingData: false}): atomic writer initialized with empty file');
